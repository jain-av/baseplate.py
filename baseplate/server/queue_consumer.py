...
class QueueConsumerServer:
    ...
    def start(self) -> None:
        ...
        logger.info("Starting pump thread.")
        self.pump_thread.start()

        logger.info("Starting handler threads.")
    ...
    def stop(self) -> None:
        ...
        Stops the healthcheck server, signals the pump to stop, signals the
        handlers to stop, and waits for the pump and handler threads to exit.
        """
        assert self.started
        if self.stopped:
            return
        ...
        # Wait for the pump and handler threads to exit.
        logger.info("Joining threads with timeout %s.", self.stop_timeout)
        self.pump_thread.join(timeout=self.stop_timeout.total_seconds())
        for thread in self.threads:
            thread.join(timeout=self.stop_timeout.total_seconds())

        if self.pump_thread.is_alive():
            logger.warning("Pump thread did not exit within timeout.")
        thread_timeouts = sum(1 for thread in self.threads if thread.is_alive())
        if thread_timeouts:
            logger.warning("%d handler threads did not exit within timeout.", thread_timeouts)
